  ...
  for (const borrow of myBorrows) {
    if (borrow.returned_at) {
      ...
      if (timeElapsedMS >= totalDaysMS) {
        ...
        // Adds fine to the borrow if it hasn't 
        if (parseFloat(borrow.borrow_fine) === 0) {
          const borrowRes = await borrowDB.update_borrow_fine(borrow.borrow_id, borrow.return_option_fine);
        }
      }
    }
  }
